...
const startServer = async (from) => {
  create("Imperial", serverOption).then((client) => {
...
async function msgHandler(client, message) {
  try {
      ...
      switch (cmd[0]) {
        case "#sticker" || "#stiker":
          if (isMedia) {
            ...
            client.sendText(
              from,
              "Tidak ada gambar! Untuk membuat sticker kirim gambar dengan caption #sticker "
            );
          }
          break;
        case "#halo":
    ...
    client.sendText(
      from,
      "Not valid image, coba kirim fotonya dengan crop sedikit atau gambar terlalu burik."
    );
  }
}
